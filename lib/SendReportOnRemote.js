...
export default class SendReportOnRemote {
  ...
  sendReport(error_data, user_id, successCallBackManager, errorCallBackManager){
    ...
    var url = Configuration.getHostURL()+'/reports';
    ...
    console.log("Error reporting url=", url);
      ...
      //  console.log('akhilesh------- ', error);
       ...
       request.setRequestHeader("os-type" , Platform.OS);
       request.setRequestHeader("app-version",  DeviceInfo.getVersion())
       request.setRequestHeader("app_id",  'PRD')
       request.setRequestHeader("Authorization", 'Bearer '+Configuration.getAPIKey());
